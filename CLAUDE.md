...
> **开发规则：任何代码修改前，必须先向用户说明方案并征求明确同意。不可自行决定直接开发。**
> - 描述要改什么、怎么改、为什么这么改
> - 等用户回复「可以」「同意」等明确确认后再动手
> - 哪怕是一个小改动（如改提示文案、调超时时间），也先征求同意
...
| practiceWrongCount | 练习累计答错次数 |
| starred | 收藏标记（独立于掌握状态） |
| reviewCount | 练习总次数 |
...
### 页面结构
- 四个阶段：`select`（筛选题目）→ `practice`（做题）→ `result`（统计）→ `review`（回顾）
- 数据来源：直接从错题本（IndexedDB `mistakes` 表）筛选，不依赖 AI 分析
- 路由：`/review`（保持兼容），底部导航标签「练习」

### 选择页（select）
- 筛选条件：模块（多选按钮）、题目类型（全部/错题/存疑）、时间范围（全部/近7天/近30天）
- 模式切换：刷题模式（每题即时反馈）/ 考试模式（全部做完统一出结果）
- 题目上限：10/20/30 道按钮选择，默认 30
- 两个开关：包含已掌握的题（默认关）、仅收藏（默认关）
- 显示匹配数量 + 随机抽取开始按钮
- 自动排除无题干或无正确答案的题

### 刷题模式（practice mode）
- 显示题干 + A/B/C/D/E 五个选项按钮
- 点选项选中（紫色），点「确认」→ 显示对错 + 正确答案 + AI 诊断解析
- 答题后显示：收藏按钮、标记已掌握按钮、练习统计（做错 X/Y 次）
- 底部：退出（弹确认） / 确认 / 下一题 / 完成
- 退出确认：显示已完成题数，确认后回到选择页，已答数据已保存

### 考试模式（exam mode）
- 显示题干 + A/B/C/D/E 五个选项按钮
- 可自由前后翻题，答案自动保存
- 显示已答/未答题数
- 交卷确认：显示未答题数警告，确认后一次性计算对错
- 交卷后进入结果页

### 计时
- 后台静默记录每道题的作答耗时，做题界面不显示任何计时器
- 只在回顾页和结果页展示耗时

### 结果页（result）
- 显示正确率（如 18/30 60%）、总耗时、平均耗时/题、最快/最慢
- 「将所有做对的标记为已掌握」按钮（批量操作）
- 查看回顾 / 再来一轮 / 返回首页

### 回顾页（review）
- 顶部统计概览 + 筛选（全部/仅错题）
- 做错的题默认展开，做对的题默认折叠
- 折叠行显示：模块标签、题干前40字、耗时、展开按钮
- 展开显示：完整题干、用户答案 vs 正确答案、用时、AI 解析、练习统计
- 每道题可：收藏、标记已掌握、跳转详情页
- 点击「详情」跳转 `/mistakes/:id`

### 数据更新
- 每道题答完后更新 `reviewCount += 1`、`reviewedAt = now`
- 答错时 `practiceWrongCount += 1`
- 收藏标记通过 `mistakeRepository.toggleStar()` 切换
- 已掌握通过 `mistakeRepository.markMastered()` 标记

### ⭐ 收藏功能
- 定义：独立于「已掌握」的标记维度。| 未掌握+已收藏=重点攻克 | 已掌握+已收藏=经典题，值得复习 |
- 标记入口：错题详情页、练习中答题后、回顾页、错题本卡片
- 筛选体现：练习选择页「仅收藏」开关、错题本「收藏」筛选项
- 数据字段：`MistakeRecord.starred?: boolean`
